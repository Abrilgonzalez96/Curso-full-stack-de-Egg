/*Dadas dos matrices cuadradas de números enteros, la matriz M de 10x10 y la matriz P de
3x3, se solicita escribir un programa en el cual se compruebe si la matriz P está contenida
dentro de la matriz M. Para ello se debe verificar si entre todas las submatrices de 3x3 que
se pueden formar en la matriz M, desplazándose por filas o columnas, existe al menos una
que coincida con la matriz P. En ese caso, el programa debe indicar la fila y la columna de
la matriz M en la cual empieza el primer elemento de la submatriz P. Como podemos observar nuestra submatriz P se encuentra en la matriz M en los índices:
4,4 - 4,5 - 4,6 - 5,4 - 5,5 - 5,6 - 6,4 - 6,5 - 6,6.*/

import java.util.Arrays;

public class ejer21 {

    public static void main(String[] args) {

        int[][] matrizM = {{1, 26, 36, 47, 5, 6, 72, 81, 95, 10}, {11, 12, 13, 21, 41, 22, 67, 20, 10, 61},
        {56, 78, 87, 90, 9, 90, 17, 12, 87, 67}, {41, 87, 24, 56, 97, 74, 87, 42, 65, 35}, {32, 76, 79, 1, 36, 5, 67, 96, 12, 11},
        {99, 13, 54, 88, 89, 90, 75, 12, 41, 76}, {67, 78, 87, 45, 14, 22, 26, 42, 56, 78}, {98, 45, 34, 23, 32, 56, 74, 16, 19, 18},
        {24, 67, 97, 46, 87, 13, 67, 89, 93, 24}, {21, 68, 78, 98, 90, 67, 12, 41, 36, 12}};
        int[][] matrizP = {{36, 5, 67}, {89, 90, 75}, {14, 22, 26}};
        LlamarFuncion.escribirMatriz(matrizM, 10, 10);
        System.out.println("");
        LlamarFuncion.escribirMatriz(matrizP, 3, 3);
        buscarMatriz(matrizM, matrizP);
    }

    public static void buscarMatriz(int[][] matrizM, int[][] matrizP) {
        for (int i = 0; i < matrizM.length; i++) {
            for (int j = 0; j < matrizM.length; j++) {
                if (matrizM[i][j] == matrizP[0][0] && i <= matrizM.length - 3 && j <= matrizM.length - 3) {
                    System.out.println("");
                    igualarMatriz(matrizM, matrizP, i, j);
                }
            }

        }
    }

    public static void igualarMatriz(int[][] matrizB, int[][] matrizP, int a, int b) {
        String[][] matrizPosicion = new String[3][3];
        int n = 0;
        int m = 0;
        int[][] matrizAux = new int[3][3];
        for (int i = a; i < a + 3; i++) {
            for (int j = b; j < b + 3; j++) {
                matrizAux[m][n] = matrizB[i][j];
                matrizPosicion[m][n] = "[" + i + ", " + j + "]";
                n++;
            }
            n = 0;
            m++;
        }

        if (Arrays.deepEquals(matrizAux, matrizP)) {
            System.out.println("La matriz se encontro en la posicion: ");
            LlamarFuncion.escribirMatrizStringSinSaltar(matrizPosicion, 3, 3);
            System.out.println("");
        }
    }
}
